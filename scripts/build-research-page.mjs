...
const tierAgree = v02val.tier_recommendation_agreement;
...
<title>Pre-KTAS 매핑 알고리즘 v0.2 — 임상 정합성 reframe</title>
<style>
  :root {
    --ink: #0a0a0a;
    --bg: #fafaf8;
...
<div class="wrap">
  ...
  <header class="top">
    <div class="stamp">EMRIS 119 · 2026-04-26</div>
    <nav>
  ...
  <h1 class="head">현장에서 구급대원이 알 수 있는 것은 정해져 있다.</h1>
  <p class="lede">그 한계 안에서, 어떤 시술은 코드만 보고도 결정된다. ${tag('Y0010')} 같은 것이다. 어떤 시술은 후보로만 좁혀진다 — ${tag('Y0020')}, ${tag('Y0031')}. 그리고 어떤 시술은 영상 검사 전엔 단정 자체가 무리다. ${tag('Y0051')}, ${tag('Y0142')}, ${tag('Y0060')}이 그렇다.</p>
  <p class="lede dim">v0.1은 27개 시술을 통계적으로 매핑하려 했고, sensitivity 0.394라는 결과로 "활용 불가" 판정을 받았다. 그 framing은 잘못됐다. 응급의학 전문의 한 명의 검토가 그것을 뒤집었다.</p>

  <section>
    <p class="frag">전문의는 이렇게 말했다.</p>
    <blockquote>
      "후보군으로 좁히려 하지 마라. 적합한 등급의 병원으로 일단 보내고, 검사한 뒤 결정하게 해라. 그게 응급의학이 작동하는 방식이다."
      <span class="who">— Phase 9b 자문, 2026-04-25, 31분</span>
    </blockquote>
    <p>이 한 문장이 다섯 시술의 분류를 바꿨다. ${tag('Y0041')}와 ${tag('Y0042')}는 후보군에서 빠져 등급 권고만 남았다. ${tag('Y0113')}도 마찬가지. 처음 분류에서 코드만으로 결정 가능하다고 본 ${tag('Y0100')}와 ${tag('Y0112')}는 분만과 함께 묶여야 한다는 의견이 추가됐다.</p>
  </section>
...
${C_codes.map(c => '      ' + tag(c, 'tier')).join('\n')}
  ...
  <section>
    ...
    <ul class="bare">
${changes.map(ch => {
  const proc = PROC[ch.y_code] || ch.y_code;
  let bandFrom = ch.from === 'A' ? 'confident' : ch.from === 'B' ? 'candidate' : 'tier';
  let bandTo = ch.to === 'A' ? 'confident' : ch.to === 'B' ? 'candidate' : 'tier';
  let direction;
  if (ch.from === 'B' && ch.to === 'C') direction = '후보군에서 빼고 등급 권고만 남김';
  else if (ch.from === 'A' && ch.to === 'B') direction = '확정에서 후보군으로 내림';
  else direction = ch.from + ' → ' + ch.to;
  return `      <li><span class="px-tag px-${bandTo}">${proc}</span> <span style="color:var(--dim);font-size:13px;">— ${direction}.</span><br><span style="font-size:14px;color:var(--dim);">${ch.rationale}</span></li>`;
}).join('\n')}
    </ul>
  </section>
  ...
  <section>
    <h2>알고리즘이 출력하는 것.</h2>
    <p>코드 하나가 들어오면 v0.2는 세 가지를 돌려준다. 어떤 시술이 확정 적응이거나 후보인지. 어떤 등급의 병원으로 가야 하는지. 추가로 물어볼 질문이 있는지.</p>
    <p>네 가지 특수 규칙도 있다. 분만 임박은 ${tag('Y0111')}을 확정으로 두고 ${tag('Y0100')}과 ${tag('Y0112')}를 함께 띄운다. 안구 천공·관통이 명시되면 ${tag('Y0160')}이 후보에서 확정으로 승격된다. ${tag('Y0150')}은 자해·정신증이 명시될 때만 trigger되며, ${tag('Y0032')}는 편마비·GCS 저하·극심한 두통이라는 구체적 소견이 있을 때만 후보로 올린다. v0.1에서 신경계 카테고리 전체가 ${tag('Y0032')}로 trigger되어 13,929건의 false positive를 만든 문제를 좁힌 것이다.</p>

    <p>4,689개 Pre-KTAS 코드를 돌린 결과:</p>
    <div class="stat-row">
      <div class="stat-item"><span class="num good">${v02.summary.by_mappability.A}</span>확정</div>
      <div class="stat-item"><span class="num">${v02.summary.by_mappability.B}</span>후보군</div>
      <div class="stat-item"><span class="num accent">${v02.summary.by_mappability.C}</span>등급만</div>
      <div class="stat-item"><span class="num">${v02.summary.by_mappability.unmapped}</span>27개 외</div>
    </div>
    <p class="note">unmapped가 80% 넘는 이유는 27개 시술이 응급실 방문의 대부분을 커버하지 않기 때문이다. 단순 두통, 발열, 경미한 외상은 27개 시술 어디에도 해당하지 않는다. 그 환자들에겐 등급 권고만 부여된다.</p>
  </section>

  <section>
    <h2>광주·전라 데이터가 말하는 것.</h2>
    <p>13만 건의 응급실 방문 데이터로 v0.2를 돌려봤다. 단, 이 데이터는 검증된 적이 없다. 자문자가 분명히 짚었다.</p>
    <blockquote>"수십% 오류가 있을 가능성이 높다. 질환별 오류율도 다르다. 통계 수치는 참고용이지 결정 근거가 아니다."</blockquote>

    <p>그래도 방향성은 본다. v0.1과 비교하면 specificity가 0.808에서 0.845로 올랐다. v0.2가 ${tag('Y0032')}와 ${tag('Y0010')} 같은 시술의 over-trigger를 좁힌 결과다. sensitivity는 0.394에서 0.329로 약간 내려갔다. 11개 시술을 후보군에서 빼고 등급 권고만 하기로 결정한 비용이다.</p>

    <p>등급 권고 일치율은 <strong>${(tierAgree.agree_rate_when_known*100).toFixed(1)}%</strong>다. 시스템이 권고한 등급 안에 환자가 실제로 도착한 등급이 포함된 비율. 사전에 등록된 임계값 0.80을 directional하게 통과한다. 다만 이 통과로 v0.2가 임상 활용 가능하다고 결론짓지는 않는다.</p>

    <p class="note">200건의 Type-A 모순이 잡혔다. 확정 매핑이 됐는데 퇴실진단 ICD-10이 일치하지 않는 케이스다. 텍스트 매칭 규칙이 너무 광범위해서일 수도 있고, 광주·전라 cohort의 코딩 품질 때문일 수도 있다. 후속 phase에서 audit 대상.</p>
  </section>

  <section>
    <h2>여기서 멈춘 자리.</h2>
    <p>이 cohort는 광주·전라에 편중돼 있다. 외부 검증이 없다. ICD-10을 27개 시술에 매핑한 것도 한 명의 임상 판단이다. 5자/6자 Pre-KTAS 코드 정렬에서 23.9%의 unique 코드가 매핑 안 됐다. 정신과 카테고리에서 ${tag('Y0150')}이 자해·정신증 키워드만으로 충분한지도 검증되지 않았다.</p>
    <p>그리고 v0.2 텍스트 매칭은 여전히 키워드 의존이다. "압박"이 ${tag('Y0010')}을 trigger하지만 흉통 맥락이 아닐 수도 있다. 200건의 Type-A 모순이 그걸 보여준다. v0.3에서 좁혀야 한다.</p>
  </section>

  <section>
    <h2>다음.</h2>
    <p>200건의 Type-A를 audit한다. 텍스트 규칙을 더 좁힌다. ${tag('Y0141')}와 ${tag('Y0142')}처럼 의도적으로 보수적인 trigger를 갖는 시술들에 대해 키워드를 약간 확장할지 검토한다. 자문자가 maintainer 위임 영역으로 남긴 추가 질문 카탈로그도 정리한다.</p>
    <p>중기로는 외부 cohort, 특히 수도권 데이터가 필요하다. 장기로는 prospective validation. 그리고 한 명이 아닌 두 명 이상의 응급의학 전문의 reference standard agreement.</p>
  </section>
  ...
  <section style="margin-top: 48px;">
    ...
    <ul class="bare">
      <li><a href="https://github.com/Pandoll-AI/emris-119chat/blob/main/research/prektas-validation-report-v2.0.md">보고서 v2.0</a> — 8섹션 본문</li>
      ...
      <li><a href="https://github.com/Pandoll-AI/emris-119chat/blob/main/research/prektas-to-y-mapping-v0.2.json">v0.2 매핑 결과</a> — 4,689 entries</li>
      <li><a href="https://github.com/Pandoll-AI/emris-119chat/blob/main/research/validation-results-v0.2.json">광주·전라 directional 통계</a> — informational</li>
      <li><a href="https://github.com/Pandoll-AI/emris-119chat/blob/main/research/mappability-review-2026-04-26-moexk8az.json">자문 원본</a> — 5건 변경, 8 질문 답변</li>
      <li><a href="https://github.com/Pandoll-AI/emris-119chat/blob/main/scripts/research/build-prektas-to-y-mapping-v0.2.mjs">v0.2 알고리즘 스크립트</a></li>
      <li><a href="https://github.com/Pandoll-AI/emris-119chat/blob/main/research/prektas-validation-report-v1.0.md">v1.0 보고서 (history)</a> — framing 폐기, 보존</li>
    </ul>
  </section>

  <footer class="foot">
    <div>Protocol PREKTAS-VAL-2026-001 · Report v2.0 · Matrix v1.0 frozen</div>
